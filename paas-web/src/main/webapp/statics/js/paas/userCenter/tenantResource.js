    ...
    app.controller('bindtenantModalCtrl', ['$scope','httpLoad','$modalInstance','$stateParams',
        function($scope, httpLoad, $modalInstance, $stateParams) {
            ...
            $scope.chooseRegion = function(item,action){
                var dataList;
                if(action==0) {
                    dataList = $scope.regionData;
                    $scope.category = "";
                }
                if(action==1) {
                    dataList = $scope.regionData2;
                    $scope.catalog = "";
                }
            ...
            $scope.getList = function(page){
                ...
                var searchParam = [{"param":{"vendorId":$stateParams.vendorId},"sign":"IN"}];
            ...
            //$scope.changeTime = function(expiredTime1){
            //    angular.forEach($scope.listData, function(data,index){
            //        data.expiredTime = expiredTime1.format('YYYY-MM-DD HH:mm:ss');
            //    });
            //};
            $scope.ok = function () {
                if(!$scope.expiredTime1||$scope.expiredTime1=="") {
                    $scope.pop('请选择过期时间','error');
                    return false;
                }
                var resources = [];
                var expiredTime = $scope.expiredTime1.format('YYYY-MM-DD HH:mm:ss');
                selectList.forEach(function (item) {
                    resources.push({resId:item.id,cpu:item.cpu,memory:item.memory,disk:item.disk,expiredTime:expiredTime})
                });
