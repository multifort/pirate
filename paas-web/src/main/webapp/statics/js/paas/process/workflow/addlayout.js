...
app.factory('NewTaskService',['$rootScope','workflowData','httpLoad','$modal', '$timeout', '$stateParams',	function ($rootScope,workflowData, httpLoad, $modal, $timeout, $stateParams) {
  ...
  NewTaskService.prototype = {
    ...
    InitTab:function(){
      var data = [{
        "nodeType": "FORK_JOIN",
        "Image": "icon_branch_5.png",
        "text": "分支"
      }, {
        "nodeType": "DYNAMIC",
        "Image": "icon_dynamic_5.png",
        "text": "动态"
      },

      {
        "nodeType": "HTTP",
        "Image": "icon_http_5.png",
        "text": "HTTP"
      },

      {
        "nodeType": "JOIN",
        "Image": "icon_merge_5.png",
        "text": "合并"
      }, {
        "nodeType": "SIMPLE",
        "Image": "icon_simpleness_5.png",
        "text": "简单"
      }, {
        "nodeType": "DECISION",
        "Image": "icon_stream_5.png",
        "text": "选择流"
      },

      {
        "nodeType": "SUB_WORKFLOW",
        "Image": "icon_workflow_5.png",
        "text": "替代工作流"
      }];
    ...
    DrawNode:function(data){
      ...
      data.nodes.forEach(function(item){
        if(item.start){
          ...
          that.AddNode(item.location.x,item.location.y,item.text,img[item.data.type],'Bottom_Center',item.data.type,item);
        }
      });
    ...
    AddTaskNode:function(node){
    	...
    	if(node.inLinks&&node.outLinks){
    		        ...
    		        var modalInstance = $modal.open({
    		          templateUrl: '/statics/tpl/process/workflow/addmodul.html',
...
app.controller('addScriptTaskModalCtrl', ['$scope','Diagramnodes','workflowData' ,'$modalInstance', '$modal', '$stateParams', '$timeout', 'httpLoad',  'node', 'CommonData',
  function ($scope,Diagramnodes,workflowData, $modalInstance, $modal, $stateParams, $timeout, httpLoad, node, CommonData) {
    $scope.modalName = node.text+"任务";
      $scope.edit = false;
    ...
    $scope.loopSys = function(node){
        if(node.inLinks.length>1){
            for(var i = 0;i<node.inLinks.length;i++ ){
                $scope.dubleLoop(node.inLinks[i].nodeA)
            }
        }else{
            if(node.inLinks[0].nodeA.nodeType == "SIMPLE"){
            ...
            }else if(node.inLinks[0].nodeA.nodeType == "DYNAMIC"){
                if(node.inLinks[0].nodeA.nodeData){
                    $scope.diagram[node.inLinks[0].nodeA.nodeData.taskReferenceName] = node.inLinks[0].nodeA.nodeData.taskReferenceName;
                }}else if(node.inLinks[0].nodeA.nodeType == "HTTP"){
                if(node.inLinks[0].nodeA.nodeData){
                    $scope.diagram[node.inLinks[0].nodeA.nodeData.taskReferenceName] =  node.inLinks[0].nodeA.nodeData.name;
                }}else if(node.inLinks[0].nodeA.nodeType == "START"){
                return
            } else{
                $scope.loopSys(node.inLinks[0].nodeA)
            }
        }
      }
      //多条循环
    ...
    $scope.getJenkins = function(name,inputdata){   //添加map样子的数组
        httpLoad.loadData({
           ...
           if(Diagramnodes.diagram){ //有分支或者选择流的情况下
             //首先找到合并点，确定他的位置；
               $scope.positionD ='';//合并点的位置
              var positionN ='' //点击点的位置

             if($scope.positionLoop(node)){//在合并点之前就一个一个找吧
                 $scope.loopSys(node)
             }else{
                 for(var i =0 ;i<Diagramnodes.nodes.length;i++){
                     $scope.eleLoop(node)
                 }
             }
             //算出点击的点的位置
        ...
        //找点击对象在合并点哪
      $scope.positionLoop = function (node) {
          if(node.inLinks[0].nodeA.nodeType=="JOIN"){
              $scope.positionD = "last"
              return true
          }else if(node.inLinks[0].nodeA.nodeType=="START") {
              return  false
          }else{
              $scope.positionLoop(node.inLinks[0].nodeA)
          }
      }
     $scope.eleLoop = function (node) {
         if(node.inLinks[0].nodeA.nodeType=="START"){
             ...
             if(node.inLinks[0].nodeA.nodeData){
                 if(node.inLinks[0].nodeA.nodeType == 'DYNAMIC'){
                     $scope.diagram[node.inLinks[0].nodeA.nodeData.taskReferenceName] = node.inLinks[0].nodeA.nodeData.taskReferenceName;
                 }else if(node.inLinks[0].nodeA.nodeType == 'SIMPLE'){
                     $scope.diagram[node.inLinks[0].nodeA.nodeData.taskReferenceName] =  node.inLinks[0].nodeA.nodeData.name;
                 }
             }
   ...
   //检查增用名
   $scope.chackName = function(name,type){
      var chackName = true ;
      if(!node.nodeData){
        switch(type/1){
      case 1:
      for(var i=0;i<workflowData.taskName.length;i++){
        if(workflowData.taskName[i]==name){
          $scope.pop("这个引用名已经存在，不能重复","error");
          chackName = false
        }
      }
        break;
      case 2:
      for(var i=0;i<workflowData.labelName.length;i++){
        if(workflowData.labelName[i]==name){
          $scope.pop("这个名称已经存在，不能重复","error");
           chackName = false
        }
      }
        break;
        case 3:
        for(var i=0;i<workflowData.jobName.length;i++){
          if(workflowData.jobName[i]==name){
            $scope.pop("这个job已经存在，不能重复","error");
             chackName = false
          }
        }
          break;
...
// 		}
 ...
 //bu需要其他参数的api接口
   $scope.goload = function(){   //添加map样子的数组
       $scope.StringData.forEach(function(item){
           if(item.type=="Api"&&item.event=='load'){
        	   httpLoad.loadData({
                   url: item.interface,
                   data:{simple:true},
                   method:item.method,
...
//需要其他参数的api接口
         ...
         success:function(data){
           if(data.success){
             ...
             $scope.modelData.forEach(function(item){
            	 ...
            	 if(item.type == "Map_select"){
            		 $scope.name_Data = item.value;
            	 }
             })
             if(node.nodeData){                     //对数据进行梳理，如果是新建并且没有填写过。弄成模板对应的样子
               ...
               $scope.modelData.forEach(function(item){
            		 if(item.type == 'Map'){
            			 var mapObj = [];
            			 var maplist = node.nodeData[item.name];
            			 ...
            			 var mapObj = [];
            			 var maplist = node.nodeData[item.name].http_request;
            			 for(var i in maplist){
            				 mapObj.push({key:i,value:maplist[i]})
            			 }
            			 item.value = 	mapObj;}
            		 else if(item.type == 'select'){
            			 if($scope.type == 'SUB_WORKFLOW'){
            				 var mapObj = [];
            				 var maplist = node.nodeData[item.name]
            				 var string = maplist.name+' / '+maplist.version
            				 mapObj.push(string);
            				 item.select = mapObj
            	        	}else{
            	        		item.select = node.nodeData[item.name];
            	        	}
            			 }else if(item.type == 'select_list'){
            			 var mapObj = [];
        				 var maplist = node.nodeData[item.name];
        				 mapObj.push(maplist);
        				 item.value = [];
        				 for(var i in node.nodeData.inputParameters){
        					 item.value.push(i)
        					 }
        				 item.select =mapObj;
            			 }else if(item.type == 'List'){
                         item.value = node.nodeData[item.name];
                     }else{
                             item.select = node.nodeData[item.name];
                         }
            	 })
    ...
    $scope.param = {};
      ...
      //对数据进行重组，，1，
        for(var m=0;m<$scope.modelData.length;m++){
          var item = $scope.modelData[m];
            if(item.type =="Map"){
                ...
                //筛选有没有重复项
                var nary=httpList.sort();
                for(var j=0;j<nary.length;j++){
                    if (nary[j]==nary[j+1] !=''){
                        $scope.pop(nary[j]+"不能重复，请重新填写",'error');
                    }
                }
                var http_request = {
                    http_request : inputParametersList
                }
                $scope.param[item.name] = http_request;
            }else if (item.type =="List") {
                $scope.param[item.name] = item.select;
            }else if (item.type =="select") {
                if($scope.type == 'SUB_WORKFLOW'){
                    var subList = item.select.split(' / ');
                    var http_request = {
                        "name": subList[0],
                        "version": subList[1]
                    }
    ...
    if($scope.type == "JOIN"){
        var inputParameters = [];
        node.inLinks.forEach(function(item){
          if(!item.nodeA.nodeData){
              $scope.pop('请添加合并前的节点信息','error');
          }else{
              inputParameters.push(item.nodeA.nodeData.taskReferenceName)
          }

        })
 
        $scope.param.joinOn = inputParameters;
      }
    if($scope.chackName($scope.param.taskReferenceName,1)){  
      if($scope.type == "HTTP"||$scope.type == "SIMPLE"){
    	  var timestamp
...
//        if($scope.chackName($scope.param.name,2)){
//            workflowData.labelName.push($scope.param.name);
//            var timestamp = Date.parse(new Date());
//            $scope.param.taskReferenceName = $scope.param.name+timestamp;
//            workflowData.taskName.push($scope.param.taskReferenceName);
//        }else{
//          return
//        }
      }else {
          $scope.param.name = $scope.param.taskReferenceName;
    	   workflowData.taskName.push($scope.param.taskReferenceName);
      }
    }else{
      return
