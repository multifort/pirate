...
public class ApplicationResource {
	...
	public ApplicationResource(MonitorClient monitorClient, EnvironmentService environmentService) {
		super();
	...
	public Application getApplication(Application application){
		...
		int serviceNum = 0;
		ServiceList serviceList = (ServiceList)client.list(namespace, ApplicationEnum.RESOURCE.SERVICE);
		if (null != serviceList) {
			serviceNum = serviceList.getItems().size();
		}
		application.setServiceNum(serviceNum);
		
		int jobNum = 0;
		JobList jobList = (JobList)client.list(namespace, ApplicationEnum.RESOURCE.JOB);
		if (null != jobList) {
			jobNum = jobList.getItems().size();
		}
		application.setJobNum(jobNum);
		
		int instanceNum = 0;
		List<Pod> pods = new ArrayList<Pod>();
		PodList podList = (PodList)client.list(namespace, ApplicationEnum.RESOURCE.POD);
		if (null != podList) {
			pods = podList.getItems();
			instanceNum = pods.size();
		}
